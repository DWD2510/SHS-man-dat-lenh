...
function selChip(c){
  ...
  document.getElementById('qfGia').style.display = ['MTL','ATO','ATC'].includes(c) ? 'none' : 'block';
  if(isMarket){
    tGia.textContent = c;
...
function validateAndRefreshForm() {
  const kl = S.vKL || 0;
  const isMarket = ['MTL', 'ATO', 'ATC'].includes(S.chip) || ['MOK', 'MAK', 'PLO'].includes(S.orderType);
  const marketLabel = ['MTL', 'ATO', 'ATC'].includes(S.chip) ? S.chip : S.orderType;
  ...
  if (kl && priceVal) {
    ...
    if (isMarket) {
      gtText += ` (Tạm tính theo ${marketLabel})`;
    }
  }
...
function updateOrderFormLayout() {
  ...
  const hideGiaForMarket = ['MTL', 'ATO', 'ATC'].includes(S.chip) || ['MOK', 'MAK', 'PLO'].includes(type);
  qfGia.style.display = hideGiaForMarket ? 'none' : 'block';
...
function selChipDM(c) {
  ...
  document.getElementById('qfDMGia').style.display = c === 'MTL' ? 'none' : 'block';
  if (isMarket) {
...
function openOrderMenu(id) {
  const o = S.orders.find(x => x.id === id);
  if (!o) return;
  _omOrderId = id;
  document.getElementById('omTitle').textContent = `${o.code} - Lệnh ${o.side === 'buy' ? 'Mua' : 'Bán'}`;
  openOv('ovOrderMenu');
}
...
function openOrderDetail(id) {
  const o = S.orders.find(x => x.id === id);
  if (!o) return;
  document.getElementById('odTitle').textContent = 'Chi tiết lệnh — ' + o.code;
  document.getElementById('odSideIcon').className = 'bsi ' + (o.side === 'buy' ? 'b' : 's');
  document.getElementById('odSideIcon').textContent = o.side === 'buy' ? 'M' : 'B';
  document.getElementById('odCode').textContent = o.code;
  document.getElementById('odTypeChip').textContent = o.type || 'LO';
  document.getElementById('odStatusTag').className = 'stag ' + SC[o.st];
  document.getElementById('odStatusTag').textContent = SL[o.st];
  document.getElementById('odGiaDat').textContent = o.p;
  document.getElementById('odGiaKhopDat').textContent = `${o.mp}/${o.pd || o.p}`;
  document.getElementById('odKLKhopDat').textContent = `${o.mq}/${o.q} CP`;
  openOv('ovOrderDetail');
}
...
function openEditOrder(id) {
  ...
  const isMarketType = ['MTL', 'ATO', 'ATC', 'MOK', 'MAK', 'PLO'].includes(o.type);

  S.editOrder = { id, code: o.code, side: o.side, type: o.type || 'LO', isMarketType };
  ...
  document.getElementById('qfEditGia').style.display = isMarketType ? 'none' : 'block';
    ...
    <div class="sln-ocard">
      ...
      <div class="sln-ocard-body">
        ...
        <div class="sln-ocard-act">${(o.st==='wait'||o.st==='partial')?`
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none" style="cursor:pointer" onclick="openOrderMenu(${o.id})"><circle cx="10" cy="4.5" r="1.6" fill="#070707"/><circle cx="10" cy="10" r="1.6" fill="#070707"/><circle cx="10" cy="15.5" r="1.6" fill="#070707"/></svg>`:''}</div>
      </div>
    </div>`).join('');
}
  ...
  el.innerHTML=slnXN.map(c=>{
    ...
    return `<div class="sln-ocard">
      <div class="sln-ocard-hd">
        ...
        <span class="sln-ocard-code">${c.code}</span>
        <span class="sln-ocard-spacer"></span>
        <span class="sln-btn-duyet" onclick="confirmApproveOne(${c.id})">Xác nhận</span>
      </div>
      <div class="sln-ocard-divider"></div>
      <div class="sln-ocard-body sln-dk-body">${c.rows.map(r=>`
